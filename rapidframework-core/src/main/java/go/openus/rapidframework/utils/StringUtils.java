...
public class StringUtils {
    ...
    public static String defaultIfEmpty(String str, String defaultValue) {
        return org.apache.commons.lang3.StringUtils.defaultIfEmpty(str, defaultValue);
    }
    ...
    public static String underlineToCamelHump(String str) {
        String lowerCaseStr = str.toLowerCase(); //先转为小写
        Matcher matcher = Pattern.compile("_[a-z]").matcher(lowerCaseStr);
        StringBuilder builder = new StringBuilder(lowerCaseStr);
        for (int i = 0; matcher.find(); i++) {
     ...
     * 转为显示命名方式（如：Foo Bar）
     */
    public static String toDisplayStyle(String str, String separator) {
        String displayName = "";
        str = str.trim().toLowerCase();
        if (str.contains(separator)) {
            String[] words = splitString(str, separator);
            for (String word : words) {
                displayName +=  firstToUpper(word) + " ";
            }
